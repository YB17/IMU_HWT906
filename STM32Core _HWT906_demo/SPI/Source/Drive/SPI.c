...
void SPIWriteBytes(SPin PinCs,unsigned short usAddr,unsigned char ucLength,unsigned char ucData[])
{
    unsigned short usTemp;
	unsigned char i;
	WriteDO(PinCs,0);
    usTemp = usAddr&0x7fff;
    i = 10;//CS 等待至少2us
    while(i--);
	SPIReadWriteByte(usTemp>>8);
	SPIReadWriteByte(usTemp&0xff);
	for (i=0;i<ucLength;i++)
	SPIReadWriteByte(ucData[i]);
	WriteDO(PinCs,1);
}
void SPIReadBytes(SPin PinCs,unsigned short usAddr,unsigned char ucLength,unsigned char ucData[])
{
    unsigned short usTemp;
	unsigned char i;
	WriteDO(PinCs,0);
